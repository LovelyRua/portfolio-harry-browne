...
export function LoginPanel(props: {
  ...
  onSuccessToken: (token: string, email: string) => void;
  onClose: () => void;
  api: ApiClient;
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      if (props.mode === 'register') {
        ...
        props.onSuccessToken(response.accessToken, email);
      }
    } catch (error) {
        ...
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-[var(--muted)]">
              Local data works without an account. Sign in when you want cloud backup across devices.
            </p>
          </div>
